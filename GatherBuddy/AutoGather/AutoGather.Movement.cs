...
namespace GatherBuddy.AutoGather
{
    public partial class AutoGather
    {
        ...
        private void MoveToCloseNode(IGameObject gameObject, Gatherable targetItem, ConfigPreset config)
        {
            ...
            if (hSeparation < 3.5)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            Navigate(gameObject.Position, false, nodeId: gameObject.DataId);
                        }
                    }
                }
            }
            ...
            {
                Navigate(gameObject.Position, ShouldFly(gameObject.Position), nodeId: gameObject.DataId);
            }
        }
        ...
        private void MoveToCloseSpearfishingNode(IGameObject gameObject, Classes.Fish targetFish, FishingSpot expectedSpot)
        {
            ...
            if (hSeparation < 3.5)
            {
                ...
                {
                    TaskManager.Enqueue(() => { if (!Dalamud.Conditions[ConditionFlag.Gathering]) Navigate(gameObject.Position, false, nodeId: gameObject.DataId); });
                }
            }
        ...
        private void HandlePathfinding()
        {
            ...
            if (_navState.flying && _navState.mountingUp && Dalamud.Conditions[ConditionFlag.Mounted])
            {
                ...
                path = VNavmesh.Path.ListWaypoints()
                    ...
                    .SkipWhile(p => Vector2.DistanceSquared(new Vector2(player.X, player.Y), new Vector2(p.X, p.Y)) < 16f)
                    .ToList();
